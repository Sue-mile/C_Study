...
### 그동안 노트에 손으로 적으며 공부하느라 깃에는 따로 정리하지 못했다.  
- 이 오늘은 요 며칠동안 공부한 pointer 에 대해 적겠다.

우선 며칠만에 다시 Git에 정리하는데 있어서 pointer 부분 부터 적는 이유는, 어차피 array 부분도 같이 써야하는데 그냥 한번에 쓰면 여러일을 한번에 처리할 수 있다 생각됐기 때문이다.

1. Pointer
- 포인터는 메모리 주소를 저장하는 변수다.

        int num = 20;
        이는 곧 num 에 20 이라는 값이 저장된 것.

        int *pointer = &num;
        이라고 하면 pointer 에 저장된 값은 num의 메모리 주소라는 뜻이다.

        & 는 주소 연산자로 메모리 주소를 구한다.

        * 는 간접 참조 연산자 다.
        포인터를 이용해 포인터가 가리키를 메모리의 값에 접근할 때 사용한다.
        ...
        *pointer = 100;
        ...
        이는 a 에 5000 을 저장하고
        pointer 에 a 주소를 저장한다.
        그렇게 b 부분도 a의 값인 5000을 저장하게 된다.

        pointer c에는 100을 저장하게 됐으니
    ...
    포인터의 주소값을 출력할 때는 %p 를 사용한다.
        ...
        여기서 사용된
        void *
        는 특저 자료형이 정해지지 않는 범용 포인터 / 혹은 여러 자료형의 주소를 저장할 수 있다.
...
그리면 내가 너무나도 즐겁게 공부했던 파트, 배열과 포인트!!
...
메모리 구정은 'C' 'O' 'M' 'P' 'U' 'T' 'E' 'R' '\0'
여기서 포인터 연산을 이용해 각 문자에도 접근이 가능하다.
*text -> C
*(text + 1) -> O
... 이런식으로 가능하다.

물론 중간에 들어가는 배열 요소를 수정할 수도 잇다.

text[0] = 'X';
이라고 수정후 출력하면 XOMPUTER 이라 출력되는 걸 알 수 있따.

### 그러면 배열은 포인터 처럼 사용할 수 있고  
### 또 반대로 포인터도 배열처럼 사용할 수 있다.
...
---
    ...
    { 
        ...
        int a = *pointer + *(pointer + 3); 
        
        pointer++; 
        
        int b = *pointer + *(pointer + 3);
        int c = *pointer + 3;
